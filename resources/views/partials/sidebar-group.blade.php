    ...
    <div x-show="isGroupOpen(groupName)" x-collapse x-cloak class="overflow-hidden">
        <div class="px-4 pb-3 space-y-1">
            ...
            <template x-if="group.__endpoints && group.__endpoints.length">
                <template x-for="endpoint in group.__endpoints" :key="endpoint.name">
                    <button
                        ...
                        :class="{
                            'bg-blue-50 border-l-4 border-blue-500 text-gray-900': active && active.name === endpoint.name,
                            'hover:bg-gray-50 text-gray-700': !(active && active.name === endpoint.name)
                        }"
            ...
            <template x-for="(nestedGroup, nestedGroupName) in Object.fromEntries(Object.entries(group).filter(([k]) => k !== '__endpoints'))" :key="nestedGroupName">
                <div x-data="{ nestedOpen: false }" :class="{ 'border-neutral-200 bg-gray-50/50' : nestedOpen, 'border-transparent' : !nestedOpen }" class="duration-200 ease-out bg-white border rounded-md cursor-pointer group mt-1">
                    <button @click="nestedOpen = !nestedOpen" class="flex items-center justify-between w-full px-3 py-2 font-medium text-left select-none">
                        <span x-text="nestedGroupName" class="text-sm text-gray-600"></span>
                        <!-- Pines-style icon -->
                        <div :class="{ 'rotate-90': nestedOpen }" class="relative flex items-center justify-center w-2 h-2 duration-300 ease-out flex-shrink-0">
                            <div class="absolute w-0.5 h-full bg-gray-300 rounded-full"></div>
                            <div :class="{ 'rotate-90': nestedOpen }" class="absolute w-full h-0.5 ease duration-500 bg-gray-300 rounded-full"></div>
                        </div>
                    </button>
